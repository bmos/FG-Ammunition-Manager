...
local function breakWeapon(rSource, nodeWeapon, sWeaponName)
    if nodeWeapon and isFragile(nodeWeapon) then
        ...
        if nBroken == 0 then
            ...
            DB.setValue(nodeWeapon, 'itemdamage', 'number', math.floor(
                            nItemHitpoints / 2) + math.max(nItemDamage, 1));
            ChatManager.Message(string.format(
                                    Interface.getString(
                                        'char_actions_fragile_broken'),
                                    sWeaponName), true, rSource);
        elseif nBroken == 1 then
            DB.setValue(nodeWeapon, 'broken', 'number', 2);
            DB.setValue(nodeWeapon, 'itemdamage', 'number',
                        nItemHitpoints + math.max(nItemDamage, 1));
            ChatManager.Message(string.format(
                                    Interface.getString(
                                        'char_actions_fragile_destroyed'),
                                    sWeaponName), true, rSource);
        end
    end
end
...
function countMissedShots(nodeWeapon, nodeAmmoLink, sResult, bCountAll)
    if nodeAmmoLink then
        if bCountAll or (sResult == 'miss' or sResult == 'fumble') then -- counting misses
            DB.setValue(nodeAmmoLink, 'missedshots', 'number',
                        DB.getValue(nodeAmmoLink, 'missedshots', 0) + 1);
        end
    else
        if bCountAll or (sResult == 'miss' or sResult == 'fumble') then -- counting misses
            DB.setValue(nodeWeapon, 'missedshots', 'number',
                        DB.getValue(nodeWeapon, 'missedshots', 0) + 1);
        end
    end
end

local function writeAmmoRemaining(rSource, nodeWeapon, nodeAmmoLink,
                                  nAmmoRemaining, sWeaponName)
    if nodeAmmoLink then
        if nAmmoRemaining == 0 then
            ChatManager.Message(string.format(
                                    Interface.getString(
                                        'char_actions_usedallammo'), sWeaponName),
                                true, rSource);
...
local function isInfiniteAmmo(rSource, nodeWeapon)
    ...
    if sRuleset == "PFRPG" or sRuleset == "3.5E" then
        bInfiniteAmmo = bInfiniteAmmo or
                            EffectManager35E.hasEffectCondition(rSource,
                                                                'INFAMMO');
    elseif sRuleset == "4E" then
        bInfiniteAmmo = bInfiniteAmmo or
                            EffectManager4E.hasEffectCondition(rSource,
                                                               'INFAMMO');
    elseif sRuleset == "5E" then
        local bThrown = DB.getValue(nodeWeapon, 'type', 0) == 2
        bInfiniteAmmo = (bInfiniteAmmo and not bThrown) or
                            EffectManager5E.hasEffectCondition(rSource,
                                                               'INFAMMO');
    end
...
function ammoTracker(rSource, sDesc, sResult, bCountAll)
    ...
    if not sDesc:match('%[CONFIRM%]') and sWeaponName ~= '' then
        local nodeWeaponList = ActorManager.getCreatureNode(rSource).getChild(
                                   '.weaponlist');
        for _, nodeWeapon in pairs(nodeWeaponList.getChildren()) do
            local sWeaponNameFromNode = getWeaponName(
                                            DB.getValue(nodeWeapon, 'name', ''))
            if sWeaponNameFromNode == sWeaponName then
                if sResult == "fumble" then -- break fragile weapon on natural 1
                    local _, sWeaponNode =
                        DB.getValue(nodeWeapon, 'shortcut', '')
                ...
                if (sDesc:match('%[ATTACK %(R%)%]') or
                    sDesc:match('%[ATTACK #%d+ %(R%)%]')) and not bMelee then
                    local nodeAmmoLink = getAmmoNode(nodeWeapon)
                    local nAmmoRemaining, bInfiniteAmmo = getAmmoRemaining(
                                                              rSource,
                                                              nodeWeapon,
                                                              nodeAmmoLink)
                    if not bInfiniteAmmo then
                        writeAmmoRemaining(rSource, nodeWeapon, nodeAmmoLink,
                                           nAmmoRemaining - 1, sWeaponName)
                        countMissedShots(nodeWeapon, nodeAmmoLink, sResult,
                                         bCountAll)
                    end
                end
            end
...
local function onAttack_pfrpg(rSource, rTarget, rRoll) -- luacheck: ignore
    ...
    local bAllowCC = OptionsManager.isOption("HRCC", "on") or
                         (not bIsSourcePC and
                             OptionsManager.isOption("HRCC", "npc"));
    ...
    if rRoll.sType == "critconfirm" then
        ...
        nMissChance = tonumber(rRoll.sDesc:match("%[MISS CHANCE (%d+)%%%]")) or
                          0;
        rMessage.text = rMessage.text:gsub(" %[AC %d+%]", "");
        rMessage.text = rMessage.text:gsub(" %[MISS CHANCE %d+%%%]", "");
    else
        nDefenseVal, nAtkEffectsBonus, nDefEffectsBonus, nMissChance =
        ...
        if nAtkEffectsBonus ~= 0 then
            ...
            table.insert(rAction.aMessages,
                         string.format(sFormat, nAtkEffectsBonus));
        end
        if nDefEffectsBonus ~= 0 then
            nDefenseVal = nDefenseVal + nDefEffectsBonus;
            local sFormat = "[" .. Interface.getString("effects_def_tag") ..
                                " %+d]";
            table.insert(rAction.aMessages,
                         string.format(sFormat, nDefEffectsBonus));
        end
    end
    ...
    elseif rAction.nFirstDie == 1 then
        if rRoll.sType == "critconfirm" then
            ...
            -- for compatibility with mirror image handler, add this here in your onAttack function
            if MirrorImageHandler and rAction.nMisfire and rRoll.sType ==
                "attack" then
        ...
        -- for compatibility with mirror image handler, add this here in your onAttack function
    elseif MirrorImageHandler and rAction.nMisfire and rAction.nFirstDie <=
        rAction.nMisfire and rRoll.sType == "attack" then
        table.insert(rAction.aMessages, "[MISFIRE]");
        rAction.sResult = "miss";
        -- end compatibility with mirror image handler
    ...
    elseif nDefenseVal then
        if rAction.nTotal >= nDefenseVal then
            if rRoll.sType == "critconfirm" then
                rAction.sResult = "crit";
                table.insert(rAction.aMessages, "[CRITICAL HIT]");
            elseif rRoll.sType == "attack" and rAction.nFirstDie >=
                rAction.nCrit then
                if bAllowCC then
                    rAction.sResult = "hit";
                    rAction.bCritThreat = true;
    ...
    if ((rRoll.sType == "critconfirm") or not rAction.bCritThreat) and
        (nMissChance > 0) then
    ...
    if AmmunitionManager and OptionsManager.isOption("ATKRESULTWEAPON", "on") then
        table.insert(rAction.aMessages,
                     "with " .. AmmunitionManager.getWeaponName(rRoll.sDesc));
    end
    ...
    if AmmunitionManager then
        local nHitMargin = AmmunitionManager.calculateMargin(nDefenseVal,
                                                             rAction.nTotal)
        if nHitMargin then
            table.insert(rAction.aMessages, "[BY " .. nHitMargin .. "+]")
        end
    end
    ...
    if rRoll.sType == "critconfirm" then
        ...
        if rAction.bCritThreat then
            ...
            local nCCMod = EffectManager35E.getEffectsBonus(rSource, {"CC"},
                                                            true, nil, rTarget);
            if nCCMod ~= 0 then
                rCritConfirmRoll.sDesc =
                    string.format("%s [CONFIRM %+d]", rRoll.sDesc, nCCMod);
            else
                rCritConfirmRoll.sDesc = rRoll.sDesc .. " [CONFIRM]";
            end
            if nMissChance > 0 then
                rCritConfirmRoll.sDesc =
                    rCritConfirmRoll.sDesc .. " [MISS CHANCE " .. nMissChance ..
                        "%]";
            end
            ...
            if nDefenseVal then
                rCritConfirmRoll.sDesc =
                    rCritConfirmRoll.sDesc .. " [AC " .. nDefenseVal .. "]";
            end

            if nAtkEffectsBonus and nAtkEffectsBonus ~= 0 then
                local sFormat = "[" .. Interface.getString("effects_tag") ..
                                    " %+d]";
                rCritConfirmRoll.sDesc =
                    rCritConfirmRoll.sDesc .. " " ..
                        string.format(sFormat, nAtkEffectsBonus);
        ...
        elseif (rAction.sResult ~= "miss") and (rAction.sResult ~= "fumble") then
            ...
            -- for compatibility with mirror image handler, add this here in your onAttack function
        elseif MirrorImageHandler and (rAction.sResult == "miss") and
            (nDefenseVal - rAction.nTotal <= 5) then
            bRollMissChance = true;
            nMissChance = 0;
            -- end compatibility with mirror image handler
    ...
    elseif MirrorImageHandler and bRollMissChance then
        local nMirrorImageCount =
            MirrorImageHandler.getMirrorImageCount(rTarget);
        if nMirrorImageCount > 0 then
            if rAction.sResult == "hit" or rAction.sResult == "crit" then
                local rMirrorImageRoll =
                    MirrorImageHandler.getMirrorImageRoll(nMirrorImageCount,
                                                          rRoll.sDesc);
                ActionsManager.roll(rSource, rTarget, rMirrorImageRoll);
            elseif rRoll.sType ~= "critconfirm" then
                MirrorImageHandler.removeImage(rSource, rTarget);
                table.insert(rAction.aMessages,
                             "[MIRROR IMAGE REMOVED BY NEAR MISS]");
            end
        end
        -- end compatibility with mirror image handler
    ...
    if rTarget then
        ActionAttack.notifyApplyAttack(rSource, rTarget, rRoll.bTower,
                                       rRoll.sType, rRoll.sDesc, rAction.nTotal,
                                       table.concat(rAction.aMessages, " "));

        -- REMOVE TARGET ON MISS OPTION
        if (rAction.sResult == "miss" or rAction.sResult == "fumble") and
            rRoll.sType ~= "critconfirm" and
    ...
    if rAction.sResult == "fumble" and
        ((sOptionHRFC == "both") or (sOptionHRFC == "fumble")) then
...
local function onAttack_4e(rSource, rTarget, rRoll) -- luacheck: ignore
    ...
    local nDefenseVal, nAtkEffectsBonus, nDefEffectsBonus =
        ActorManager4E.getDefenseValue(rSource, rTarget, rRoll);
    if nAtkEffectsBonus ~= 0 then
        rAction.nTotal = rAction.nTotal + nAtkEffectsBonus;
        local sFormat = "[" .. Interface.getString("effects_tag") .. " %+d]"
    ...
    if rAction.nFirstDie >= 20 then
        ...
        if nDefenseVal then
            ...
            table.insert(rAction.aMessages,
                         "[AUTOMATIC HIT, CHECK FOR CRITICAL]");
        end
    elseif rAction.nFirstDie == 1 then
        rAction.sResult = "fumble";
        table.insert(rAction.aMessages, "[AUTOMATIC MISS]");
...
local function onAttack_5e(rSource, rTarget, rRoll) -- luacheck: ignore
    ...
    local nDefenseVal, nAtkEffectsBonus, nDefEffectsBonus =
        ActorManager5E.getDefenseValue(rSource, rTarget, rRoll);
    if nAtkEffectsBonus ~= 0 then
        rAction.nTotal = rAction.nTotal + nAtkEffectsBonus;
        local sFormat = "[" .. Interface.getString("effects_tag") .. " %+d]"
        table.insert(rAction.aMessages, string.format(sFormat, nAtkEffectsBonus));
...
function onInit()
    ...
    if sRuleset == "PFRPG" or sRuleset == "3.5E" then
        tLoadWeapons = {
            'loadaction', 'firearm', 'crossbow', 'javelin', 'ballista',
            'windlass', 'pistol', 'rifle', 'sling'
        };
    ...
    OptionsManager.registerOption2('ATKRESULTWEAPON', false,
                                   'option_header_game',
                                   'opt_lab_atkresultweaponname',
                                   'option_entry_cycler', {
